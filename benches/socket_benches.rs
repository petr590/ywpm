...
use ywpm::client::writer::write_string_vec;
use ywpm::server::reader::read_string_vec;
...
fn client_v3(stream: &mut UnixStream, vec: &Vec<String>) -> Result<(), Box<dyn Error>> {
    ...
    write_string_vec(&mut slice, vec)?;
