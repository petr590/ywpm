...
use crate::daemon::display_mode::{AlignX, AlignY, DisplayMode, DisplayModeParseError, FitMode};
...
const CONTAIN_LEFT_TOP:     DisplayMode = DisplayMode::new_with_fields(FitMode::Contain, AlignX::Left,   AlignY::Top);
const CONTAIN_RIGHT_CENTER: DisplayMode = DisplayMode::new_with_fields(FitMode::Contain, AlignX::Right,  AlignY::Center);
const CONTAIN_CENTER_TOP:   DisplayMode = DisplayMode::new_with_fields(FitMode::Contain, AlignX::Center, AlignY::Top);
...
fn display_mode_parses() -> Result<(), DisplayModeParseError> {
    ...
    assert_eq!(DisplayMode::from_str("contain right center")?, CONTAIN_RIGHT_CENTER);
    assert_eq!(DisplayMode::from_str("contain center top")?,   CONTAIN_CENTER_TOP);
    assert_eq!(DisplayMode::from_str("top contain center ")?,  CONTAIN_CENTER_TOP);
    Ok(())
}