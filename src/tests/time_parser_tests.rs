...
fn check_time_parses_normally() -> Result<(), ArgParseError> {
    ...
    let date_time = NaiveDate::from_ymd_opt(2026, 7, 27)
        .unwrap()
        .and_hms_opt(20, 8, 0)
        .unwrap();
    ...
    assert_eq!(
        parse_time_to_minutes("2026-07-27 20:08:32.123457890")?,
        date_time
    );

    assert_eq!(
        parse_time_to_minutes("2026-07-27T20:08:32.123457890Z")?,
        date_time
    );
    assert_eq!(
        parse_time_to_minutes("27 Jul 2026 20:08:32 +0000")?,
        date_time
    );
    assert_eq!(
        parse_time_to_minutes("Mon, 27 Jul 2026 20:08:32 +0000")?,
        date_time
    );
...
fn check_duration_parses_normally() -> Result<(), ArgParseError> {
    ...
    assert_eq!(
        parse_duration("32:59")?,
        Duration::hours(32) + Duration::minutes(59)
    );
